...
## navigate to page
rD$navigate("https://gofossilfree.org/divestment/commitments/")

# Check for webpage scraping legality
#paths_allowed(paths = c("https://gofossilfree.org/divestment/commitments/"))

## how to extract data? navigate the DOM
## Let's work on country demographics first
demoTable <- rD$findElement(using = "css", "#section_5")
...
demoTableRowEntries <- map(demoTableRows, function(t) {
        ...
        Name_of_Organisation <- entries[[1]]$getElementAttribute("innerHTML")[[1]]
        Type <- entries[[2]]$getElementAttribute('innerHTML')[[1]]
        Country <- entries[[3]]$getElementAttribute('innerHTML')[[1]]
        Divestment_Type <- entries[[4]]$getElementAttribute('innerHTML')[[1]]
        More_Info <- entries[[5]]$getElementAttribute('innerHTML')[[1]]
        return(data.frame(Name_of_Organisation = Name_of_Organisation, Type = Type, Country = Country, Divestment_Type = Divestment_Type, More_Info = More_Info))
})
